...
if(trim($order_detail['shipping_address1'])) {
	$sales_confirmation_pdf_content = str_replace(array('{billing_address_fields.if_blank_then_hide-start}','{billing_address_fields.if_blank_then_hide-end}'),array('',''), $sales_confirmation_pdf_content);
} else {
	$sales_confirmation_pdf_content = preg_replace('#\{billing_address_fields.if_blank_then_hide-start}.*?\{billing_address_fields.if_blank_then_hide-end}#s', '$1  $3', $sales_confirmation_pdf_content);
}
...
$pdf->Output(CP_ROOT_PATH.'/media/pdf/receipt-'.$random_mix_number.'-'.$order_id.'.pdf', 'I');
?>

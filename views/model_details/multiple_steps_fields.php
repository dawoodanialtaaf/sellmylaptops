...
<div class="block phone-details position-relative m-detail clearfix pb-0 multi_step_uniform_section">
	<div class="card">
		<?php 
		...
		<div class="row">
			<div class="col-md-4 phone-image-block text-center">
				...
				echo '<h4 class="device-name-dt"><span class="device_brand_name">'.$model_data['title'].'</span> '.($tooltip_device_content?'<span class="tips" data-toggle="modal" data-target="#device_info_popup"><i class="ml-1 fas fa-question-circle"></i></span>':'').' <span class="device-name"></span></h4>'; ?>
			</div>
			<div class="col-md-8">
                <div class="step-details" id="device-prop-area">
					...
					while($row_cus_fld = mysqli_fetch_assoc($exe_cus_fld)) {
						...
						if($row_cus_fld['type'] == "condition") {
							...
							$is_condition = '1';
						}/* elseif($row_cus_fld['type'] == "color") {
							$fld_css_class = "phone-color";
							$fld_css_sub_class = "";
						}*/ ?>

						<div class="phone-details-m-steps model-details-panel <?=$fld_css_class?>" id="fld-<?=$fld_id?>" data-row_type="<?=$input_type?>" data-required="<?=$row_cus_fld['is_required']?>" data-is_condition="<?=$is_condition?>">
							<?php /*?><h4 class="<?=$fld_title_css_class?>">
								<?php
								echo $fld_num.". ".$fld_title;
								if($row_cus_fld['tooltip']!="" && $tooltips_of_model_fields == '1') { ?>
								   <a data-toggle="tooltip" data-placement="top" title="<?=$row_cus_fld['tooltip']?>" data-original-title="<?=$row_cus_fld['tooltip']?>"><span class="fa fa-info-circle"></span></a> 
								<?php
								} ?>
							</h4><?php */?>
							...
							<div class="h4 <?=$fld_title_css_class?>">
								<?php
								echo $fld_num.". ".$row_cus_fld['title'];
													...
													if($tooltips_of_model_field_options == '1') {
														...
														<div class="condition-block">
															...
															if($enabled_any_all_in_condition_heading == '1') {
																if($row_cus_opt['tooltip_type']=="any") {
																	echo '<h3>'._lang('any_condition_heading_text','model_details').'</h3>';
																} elseif($row_cus_opt['tooltip_type']=="all") {
																	echo '<h3>'._lang('all_condition_heading_text','model_details').'</h3>';
																}
															}
															echo $field_option_tooltip_data['tooltip'];
															//echo $row_cus_opt['tooltip']; ?>
														</div>		
														<?php
														}
													} ?>
									...
									/*elseif($row_cus_fld['type'] == "color") {
										while($row_cus_opt = mysqli_fetch_assoc($exe_cus_opt)) {
											...
											elseif($row_cus_opt['is_default'] == '1') {
											  ...
											  <div class="custom-control custom-radio color_item">
												<input type="radio" name="<?=$fld_title.':'.$fld_id?>" id="<?=$fld_title_slug.'-'.$row_cus_opt['id']?>" value="<?=$row_cus_opt['label'].':'.$row_cus_opt['id']?>" autocomplete="off" data-default="<?=$row_cus_opt['is_default']?>" <?php if($row_cus_opt['is_default'] == '1'){echo 'checked="checked"';}?> <?=$checked_prms?> class="custom-control-input radio_select_buttons">
												<label class="custom-control-label" for="<?=$fld_title_slug.'-'.$row_cus_opt['id']?>" id="label-<?=$fld_title_slug.'-'.$row_cus_opt['id']?>">
													<?php
													echo '<span class="color-box" style="background-color:'.$row_cus_opt['color'].';"><i class="fas fa-check"></i></span> '.$row_cus_opt['label']; ?>
												</label>
											  </div>
										<?php }
					...
					<div class="phone-details-m-steps model-details-panel" id="get-offer" data-row_type="offer" data-required="0">
						...
						<?php /*?><div class="row pt-3 pb-2">
							<div class="col-md-6">
								<h4 class="text-uppercase">Your Device is value at</h4>
								<h4 class="total-price show_final_amt"></h4>
								<p>21 days price lock guarantee</p>
								<div class="row">
									<div class="col-md-12 text-center">
										<span><?=_lang('quantity_text','model_details')?></span> <input type="number" class="form-control" id="pc_quantity" value="<?=$saved_quantity?>" min="1" max="999" style="width:100px;display:inline;"/>
									</div>
								</div>
							</div>
							<div class="col-md-6 text-right">
								<img class="pb-4 sealed-logo" src="<?= SITE_URL ?>media/images/1GuyGadgetRibbon.png" alt="">
							</div>
						</div>
						<div class="row pt-4 border-divider">
							...
							<div class="col-md-10 col-9">
								<ul>
									<li>
										<h3>We do not buy lost or stolen devices.</h3>
										<a href="#" class="suppot-ques" data-toggle="modal" data-target="#IsMyDeviceBlackListed">Not sure? Here's how to check</a>
									</li>
								</ul>
							</div>
						</div>
						...
						<div class="custom-control custom-checkbox pt-4 pb-4">
							...
							<label class="custom-control-label" for="device_terms">My device does not violate the above warnings.</label>
							<div class="clearfix">
					...
					<div class="step-navigation clearfix" style="display:none;">
						<a href="javascript:void(0)" id="prevBtnCon" class="float-left btn btn-lg btn-secondary"><?=_lang('back_button_text')?></a>
						<a href="javascript:void(0)" id="prevBtn" class="float-left btn btn-lg btn-secondary"><?=_lang('back_button_text')?></a>
						<a href="javascript:void(0)" id="nextBtnCon" class="float-right btn btn-lg btn-primary"><?=_lang('next_button_text')?></a>
						<a href="javascript:void(0)" id="nextBtn" class="float-right btn btn-lg btn-primary"><?=_lang('next_button_text')?></a>
						<a href="javascript:void(0)" id="getOfferBtn" class="float-right btn btn-lg btn-primary"><strong><?=($show_instant_price_on_model_criteria_selections == '1'?_lang('next_button_text'):_lang('get_offer_button_text','model_details'))?></strong></a>
					</div>
					<?php
					} ?>
				</div>
...
function get_price(event_type = "") {
	tpj.ajax({
		type: 'POST',
		url: '<?=SITE_URL?>ajax/get_model_price.php?event_type='+event_type,
		...
		success: function(data) {
			if(data!="") {
				...
				var is_get_offer_page = tpj('#is_get_offer_page').val();

				console.log("total_price: ",total_price);

				if(total_price > 0) {
					tpj("#is_amt_appear").val(1);
					if(show_instant_price_on_model_criteria_selections == '1') {
						if(is_get_offer_page == '1') {
...
function con_field_next_step() {
	...
	var last_prev_id = tpj(last_step).prev().attr("id");
	
	var current_step = tpj(".model-details-panel:visible");
	var current_id = tpj(current_step).attr("id");
	
	tpj('.conditions_list_s_h').hide();
	tpj('.condition-block').show();
	
	tpj('.model-details-panel:visible .radio_select_con_buttons').each(function(i, element) {
		if(tpj(this).is(':checked')) {
			tpj(this).parent().show();
		}
	});

	if(current_id == last_prev_id) {
		tpj('#getOfferBtn').show();
		tpj('#nextBtn').hide();
		tpj("#nextBtnCon").hide();
	} else {
		tpj("#nextBtn").show();
	...
	if(show_instant_price_on_model_criteria_selections == '1') {
		tpj(".device_value_style").css("margin-top", "-65px");
	}
}
...
function field_next_step() {
	...
	var is_condition = tpj(next_step).attr("data-is_condition");
	var row_type = tpj(next_step).attr("data-row_type");
	if(is_condition == '0' && (row_type == "radio" || row_type == "select")) {
		var showhide_next_btn = false;
		tpj('.model-details-panel:visible .radio_select_buttons').each(function(i, element) {
			if(tpj(this).is(':checked')) {
	...
	if(is_condition == '0' && (next_id == last_prev_id || current_id == last_prev_id)) {
		tpj('#getOfferBtn').show();
		tpj('#nextBtn').hide();
		tpj('#nextBtnCon').hide();
	}
...
function con_field_prev_step() {
	...
	tpj('.condition-block').hide();
	tpj('.conditions_list_s_h').show();

	var is_condition = tpj(prev_step).attr("data-is_condition");
	var row_type = tpj(prev_step).attr("data-row_type");
	
	if(current_id == first_id) {
		tpj('#prevBtnCon').hide();
		tpj('#prevBtn').hide();
	} else if(is_condition == '0') {
		tpj('#prevBtnCon').hide();
		tpj('#prevBtn').show();
	}
...
function field_prev_step() {
	...
	if(is_condition == '0' && prev_id == last_prev_id) {
		tpj('#getOfferBtn').show();
		tpj('#nextBtn').hide();
	} else if(is_condition == '1') {
		tpj('#getOfferBtn').hide();
...
tpj(document).ready(function($) {
	...
	$("#nextBtnCon").click(function() {
		...
		var is_c_true = "yes";
		tpj('.model-details-panel:visible .radios').each(function(i, element) {
			var is_c_valid = tpj(this).parent().parent().find("input:checked").length;
			if(is_c_valid==0) {
				is_c_true = "no";
			}	
		});
	...
	$("#prevBtnCon").click(function() {
		con_field_prev_step();
	});

	$("#prevBtnFCart").click(function() {
		tpj('#is_get_offer_page').val(0);
		
		var is_amt_appear = $('#is_amt_appear').val();
		if(is_amt_appear == '1') {
			tpj('.show_device_value_section').show();
		}
	...
	$('#device-prop-area .radio_select_con_buttons').bind('click', function() {
		price_updt_spining_icon();
		con_field_next_step();
		setTimeout(function(){
			get_price();

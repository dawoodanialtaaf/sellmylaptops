...
function show_model_series_question_div(){
					...
					question_div+='<div class="col-md-12">';
						question_div+='<div class="clearfix">';
							question_div+='<div class="question assistant d-flex float-left">';
								...
								question_div+='<img class="assistant" src="<?=SITE_URL?>media/images/assistantForQuestion-small.svg" alt="">';
								question_div+='<p><small><?=SITE_NAME?></small></p>';
								question_div+='</div>';
								question_div+='<div class="loading" id="spinner" style="display:none"></div>';
								question_div+='<div id="msg_div" style="display:none">';
									question_div+='<span>'+question_title+''+field_tooltip_html+'</span>';
								question_div+='</div>';
							question_div+='</div>';
						question_div+='</div>';
					question_div+='</div>';
									...
									product_options_list.forEach(function(option_item,io){
										...
										ans_div+='<span class="options_values">';
											if(input_type=="select" || input_type=="radio") {
												if(type == "condition") {
													ans_div+='<button data-issubmit="'+$last_next_button+'" class="btn btn-sm capacity-row model_radio_select_btn radio_select_con_buttons" data-opt_id="'+product_opt_id+'" type="button" data-input_type="'+input_type+'" data-current_i="'+i+'">'+label+'</button>';
													ans_div+=' <input class="radioele" name="'+_new_field_name+'" value="'+_new_field_value+'" type="radio" style="display:none;" data-price="'+price+'" data-add_sub="'+add_sub+'" data-price_type="'+price_type+'" data-default="'+is_default+'" data-issubmit="'+$last_next_button+'" data-input_type="'+input_type+'" data-current_i="'+i+'" ';
													/*if (checked){
												...
												ans_div+='<div class="custom-control custom-radio custom-control-inline checkbox checkbox-success"><input class="checkboxele custom-control-input" name="'+_new_field_name+'[]" id="'+label+'" value="'+_new_field_value+'" type="checkbox" data-price="'+price+'" data-add_sub="'+add_sub+'" data-price_type="'+price_type+'" data-default="'+is_default+'"/>';
	...
	jQuery(function() {
		...
		jQuery(document).on('click', '.brand_id', function() {
			...
			if(device_list>1){
				...
				show_model_series_question_div();
			}
			else{

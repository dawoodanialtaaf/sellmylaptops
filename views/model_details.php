...
<section id="breadcrumb" class="<?=$active_page_data['css_page_class']?> py-0">
    <div class="container-fluid">
		...
		<div class="col-md-10">
		 ...
		 // Wrap the model title in a link that opens Google search with the model title and sellmylaptops.com as the query
			$model_title = htmlspecialchars($model_data['title']); // Sanitize title to avoid XSS
			$search_query = urlencode("Sell your ".$model_title . " sellmylaptops.com"); // Append sellmylaptops.com to the search query
			echo '<h1><a style="color:#000;font-size:30px;" href="https://www.google.com/search?q=' . $search_query . '" target="_blank">'
				. _lang('model_title_before_text', 'model_details') 
		 ...
		 <div class="col-md-2 mt-4">
		    <?php
				if(isset($_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_REFERER']) {
					echo '<a class="btn btn-primary mt-2 back-button" style="border:2px solid white;" href="javascript:void(0);" onclick="history.back();">'._lang('back_button_text').'</a>';
				}
				?>
		 </div>
		</div>
...
<section id="model_details" class="">
    <div class="container-fluid">
...
<?php
if(!empty($tooltip_device_content)) { ?>
	<div class="modal fade common_popup" id="device_info_popup" role="dialog">
		<div class="modal-dialog small_dialog">
			<!-- Modal content-->
			<div class="modal-content">
				<button type="button" class="close" data-dismiss="modal"><img src="<?=SITE_URL?>media/images/payment/close.png" alt="Popup Close"></button>
				<div class="modal-body">
					<?=nl2br($tooltip_device_content)?>
				</div>
			</div>
		</div>
	</div>
...
foreach($cus_opt_tooltips_data_arr as $cus_opt_tooltips_dt) { ?>
	<div class="modal fade common_popup" id="defect_examples_model_<?=$cus_opt_tooltips_dt['id']?>" role="dialog">
		<div class="modal-dialog small_dialog">
			<div class="modal-content">
				<button type="button" class="close" data-dismiss="modal"><img src="<?=SITE_URL?>media/images/payment/close.png" alt="Popup Close"></button>
				<div class="modal-body">
					<h3 class="title"><?=$cus_opt_tooltips_dt['name']?></h3>
					<?=$cus_opt_tooltips_dt['tooltip']?>
				</div>
			</div>
		</div>
	</div>
...
$model_description = str_replace("<p><br></p>","",$model_data['description']);
if(!empty($model_description)) { ?>
	<section> 
		<div class="container-fluid">
			<div class="block setting-page clearfix">
				<div class="wrap">
					<?=nl2br($model_description)?>
				</div>
			</div>
		</div>		
	</section>
...
<div class="modal fade common_popup" id="condition_examples_model" role="dialog">
			...
			<div class="container">
				...
				<ul class="nav nav-tabs" role="tablist">
					...
					foreach($fields_options_tooltips_arr as $fot_k => $fields_options_tooltips_dt) {
						if($fields_options_tooltips_dt['cond_image']) {
							$fot_n = $fot_n+1; ?>
							<li class="nav-item" style="width:75px;">
								<a style="min-height:30px;" class="nav-link<?=($fot_n == '1'?' active':'')?>" data-toggle="tab" href="#cond_image_<?=$fot_k?>"><?=$fields_options_tooltips_dt['title']?></a>
							</li>
						<?php
						}
					} ?>
				...
				<div class="tab-content">
					...
					foreach($fields_options_tooltips_arr as $fot_k => $fields_options_tooltips_dt) {
						if($fields_options_tooltips_dt['cond_image']) {
							$fot_n = $fot_n+1; ?>
							<div id="cond_image_<?=$fot_k?>" class="container tab-pane<?=($fot_n == '1'?' active':' fade')?>"><br>
								<p>
									<img <?php /*?>style="width:200px;"<?php */?> src="<?=SITE_URL?>media/images/categories/<?=$fields_options_tooltips_dt['cond_image']?>" alt="<?=$fields_options_tooltips_dt['title']?>">
								</p>
							</div>
						<?php
						}
...
<div class="modal fade common_popup" id="commonly_missed_defects_model" role="dialog">
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
	    <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
        ...
        <div class="modal-body">
		  <h4>Commonly Missed Defects Video</h4>
		  <div class="commonly_missed_defects_video_div">
		  	<iframe width="540" height="300" id="commonly_missed_defects_video" src="" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
		  </div>
        </div>
      </div>
    </div>
